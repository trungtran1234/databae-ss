...
2. If the input does not mention anything related to the database or database schema or the input mentions a table or data that does not 
actually exist in the database, return "NOT_APPLICABLE".
3. If the input is asking for a data-related request that can be fulfilled by generating an SQL query (e.g., asking for specific data, filtering data, or aggregating results), return "IS_QUERY".
4. If the input is asking for a general explanation, theoretical discussion, or any other non-actionable information, return "NOT_QUERY".
5. Do not attempt to generate the SQL query or explanation yourself. Your task is only to categorize the user's input.
6. Make sure the decision is clear and accurate to prevent misclassification.
"""